...
{//Rq: La pile ne contient que les operateurs et '('
	//affiche(exp);
	Pile p;
	Noeud *l = exp;
	int val;
	...
	while (exp != 0)
	{
		...
		{//on a un operateur 
			...
			{
				val = depiler(&p);
				c_enfiler(sortie, val);
			}

			c_empiler(&p, exp->info);
		}else if(exp->info=='('){ 
			empiler(&p, exp->info);

		}else if (exp->info==')')
		{//Depiler tous les operateurs jusqu'a '('
			while (1)
			{
				if (pile_est_vide(p))
				{
				...
				{
					val = depiler(&p);
					c_enfiler(sortie, val);//enfiler l'operateur
				}else{
					val = depiler(&p);
					break;
				}
		...
		}else{//on a un nombre on l'enfile
			if (exp->type==INT)
			{
				enfiler(sortie, exp->info);
			}else{
				f_enfiler(sortie, exp->f_info);
			}
		...
		*/
		exp = exp->suivant;
	}//depiler tous les operateurs restants
...
float f_calcul(int operateur, float operande_1, float operande_2)
{
	switch (operateur)
	{
	case '+':
...
float evaluation_gauche_droite(File exp_post_fixe)
{
	//On empile uniquement les nombres
	Pile p;
	int val, operateur, operande_1, operande_2, resultat;
	float operande_1f, operande_2f;
	float valf=0;float resultat_f;
	while (!file_est_vide(exp_post_fixe))
	{
		if (exp_post_fixe.liste->type==INT)
		{
		...
		}else if (exp_post_fixe.liste->type==FLOAT)
		{
			valf= f_defiler(&exp_post_fixe);
			//printf("%f",valf);
		...
		}else
		{
			...
			//determine operande1
			if(p.liste->type ==INT){
					operande_1f = depiler(&p);
					//printf("   %d  ",operande_1f);
			}else{
					operande_1f = f_depiler(&p);
					//printf("   %f  ",operande_1f);
					
			}
			//determine operande2
			if(p.liste->type ==INT){
					operande_2f = depiler(&p);
					//printf("   %d  ",operande_2f);
			}else{
					operande_2f = f_depiler(&p);
					//printf("   %f  ",operande_2f);
					
			}
			operateur=val;
			//A revoir
			resultat_f = f_calcul(operateur, operande_1f, operande_2f);
			//printf("%f",resultat_f);
			f_empiler(&p, resultat_f);
		}
...
int main(int argc, char **argv)
{
	...
	*/
	...
	affiche(exp_arith);
	int test=exp_post_fixe_avec_prio(exp_arith, &sortie);
	if (test!=0)
	{
	...
	resultat2_f = evaluation_gauche_droite(sortie);
	printf("Avec priorité: %f\t ", resultat2_f);
/*
